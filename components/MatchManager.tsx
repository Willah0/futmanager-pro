...
export const MatchManager: React.FC<Props> = ({
  ...
  // Goalkeeper Selection State (Used during prep)
  const [selectedGkA, setSelectedGkA] = useState<string | 'improvisado'>('improvisado');
  const [selectedGkB, setSelectedGkB] = useState<string | 'improvisado'>('improvisado');
  ...
  // Tactical Shuffle Logic - Distributes ALL players line players
  const shuffleTeams = () => {
    // Separate Goalkeepers from Line Players
    const gks = presentPlayers.filter(p => p.positions.includes('Goleiro'));
    const linePlayers = presentPlayers.filter(p => !p.positions.includes('Goleiro'));

    if (linePlayers.length < 2) return;

    const shuffle = <T,>(arr: T[]): T[] => [...arr].sort(() => Math.random() - 0.5);

    // Create copy of pool (LINE players only)
    let pool = shuffle([...linePlayers]);

    const tA: Player[] = [];
    const tB: Player[] = [];

    // Helper to create improvised player
    const createImprovisedGK = (team: 'A' | 'B'): Player => ({
      id: `improvised-gk-${team}-${Date.now()}`,
      name: 'IMPROVISADO',
      positions: ['Goleiro'],
      type: 'Diarista',
    ...
    // Add selected GKs if they exist as real players, otherwise add placeholder
    const actualGkA = gks.find(p => p.id === selectedGkA);
    const actualGkB = gks.find(p => p.id === selectedGkB);

    tA.push(actualGkA || createImprovisedGK('A'));
    tB.push(actualGkB || createImprovisedGK('B'));
    ...
    const getBestFitTeam = (p: Player, countA: number, countB: number): 'A' | 'B' => {
      // Balance counts first (ignoring GK for calculation of line imbalance)
      const lineA = tA.filter(p => !p.positions.includes('Goleiro')).length;
      const lineB = tB.filter(p => !p.positions.includes('Goleiro')).length;

      if (lineA < lineB) return 'A';
      if (lineB < lineA) return 'B';
    ...
    // 1. (GKs already skipped or assigned)
    ...
    // If we have a manual GK, he's always a starter
    ...
    // Initialize Match
    const reasoningText = `Sorteio realizado excluindo goleiros. Titulares definidos por ordem de chegada.`;
  ...
  const handleAiSort = async () => {
    ...
    try {
      // Pass ONLY line players to AI
      const result = await generateSmartTeams(
        linePlayers,
        settings.playersPerTeam, // This is still the total team size
        history,
        settings.tacticalSchema
      );

      const mapNamesToPlayers = (names: string[]) => {
        return names.map(name => linePlayers.find(p => p.name === name)).filter(Boolean) as Player[];
      };

      const tA = mapNamesToPlayers(result.teamA);
      const tB = mapNamesToPlayers(result.teamB);

      // Helper to create improvised player (reused from shuffle logic or defined here)
      const createImprovisedGK = (team: 'A' | 'B'): Player => ({
        id: `improvised-gk-${team}-${Date.now()}`,
        name: 'IMPROVISADO',
        positions: ['Goleiro'],
      ...
      // Add selected GKs manually
      const actualGkA = gks.find(p => p.id === selectedGkA);
      const actualGkB = gks.find(p => p.id === selectedGkB);

      tA.unshift(actualGkA || createImprovisedGK('A'));
      tB.unshift(actualGkB || createImprovisedGK('B'));
          ...
          <div className="flex flex-col truncate w-full">
            <div className="flex justify-between items-center w-full">
              <div className="flex items-center gap-1 truncate">
                <span className="font-medium text-base truncate">
                  {player.id.startsWith('improvised-') ? 'IMPROVISADO' : player.name}
                </span>
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              {isStarter && subData && (shouldLeave || subData.reason.includes('Protegido')) && (
                <span className={`text-[9px] italic text-right truncate max-w-[100px] ${subData.score < 0
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 text-center">
          ...
          <div className="mb-6 space-y-4">
            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg inline-block min-w-[200px]">
              <p className="text-gray-500 dark:text-gray-400 text-sm uppercase">Jogadores de Linha</p>
              <p className="text-3xl font-bold text-pitch-600 dark:text-pitch-400">
                {presentPlayers.filter(p => !p.positions.includes('Goleiro')).length}
              </p>
            </div>

            {/* Goalkeeper Manual Selection */}
            <div className="text-left bg-white dark:bg-gray-800 border rounded-lg p-4 shadow-sm">
              <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3 uppercase flex items-center gap-2">
                <ShieldCheck size={16} className="text-yellow-500" /> Escolha dos Goleiros
              </h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Team A GK */}
                <div className="space-y-2">
                  <label className="text-xs font-medium text-gray-500">Goleiro Time A</label>
                  <select
                    value={selectedGkA}
                    onChange={(e) => setSelectedGkA(e.target.value)}
                  ...
                  >
                    ...
                    {presentPlayers.filter(p => p.positions.includes('Goleiro')).map(gk => (
                      <option key={gk.id} value={gk.id} disabled={gk.id === selectedGkB}>
                        {gk.name}
                      </option>
                    ))}
                  </select>
                ...
                {/* Team B GK */}
                <div className="space-y-2">
                  <label className="text-xs font-medium text-gray-500">Goleiro Time B</label>
                  <select
                    value={selectedGkB}
                    onChange={(e) => setSelectedGkB(e.target.value)}
                    className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                  >
                    <option value="improvisado">-- IMPROVISADO --</option>
                    {presentPlayers.filter(p => p.positions.includes('Goleiro')).map(gk => (
                      <option key={gk.id} value={gk.id} disabled={gk.id === selectedGkA}>
                        {gk.name}
                      </option>
                    ))}
                  </select>
          ...
          <div className="flex flex-col gap-4 justify-center">
            <button
              ...
              disabled={presentPlayers.filter(p => !p.positions.includes('Goleiro')).length < 2}
            ...
            <button
              ...
              disabled={presentPlayers.filter(p => !p.positions.includes('Goleiro')).length < 2 || isGenerating}
          ...
          <p className="mt-4 text-xs text-gray-400">
            O sistema distribuirá os JOGADORES DE LINHA. Goleiros são fixos conforme selecionado acima.
          </p>
        </div>
      </div>
    );
